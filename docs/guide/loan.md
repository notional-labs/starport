---
description: Loan blockchain using Tinyport
order: 6
...
In this tutorial, you learn about a basic loan system as you use Tinyport to build a loan module.
...
Use Tinyport to scaffold a fully functional Cosmos SDK blockchain app named `loan`:

```bash
tinyport scaffold chain github.com/cosmonaut/loan --no-module
```
...
```bash
tinyport scaffold module loan --dep bank
```
...
Use the [scaffold list](https://docs.tinyport.com/cli/#tinyport-scaffold-list) command to scaffold code necessary to store loans in an array-like data structure:

```bash
tinyport scaffold list loan amount fee collateral deadline state borrower lender --no-message
```
...
You can use the `tinyport scaffold message` command to create each of the messages. 
...
```bash
tinyport scaffold message request-loan amount fee collateral deadline
```
...
```bash
tinyport scaffold message approve-loan id:uint
```
...
```bash
tinyport scaffold message repay-loan id:uint
```
...
```bash
tinyport scaffold message liquidate-loan id:uint
```
...
```bash
tinyport s message cancel-loan id:uint
```
