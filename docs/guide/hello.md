...
In the previous chapter you've learned how to install [Tinyport](https://github.com/notional-labs/tinyport), the tool that offers everything you need to build, test, and launch your blockchain with a decentralized worldwide community.

This series of tutorials is based on a specific version of Tinyport, so to install Tinyport v0.19.2 use the following command:

```bash
curl https://get.tinyport.network/tinyport@v0.19.2! | bash
```

Tinyport comes with a number of scaffolding commands that are designed to make development easier by creating everything that's required to start working on a particular task.

One of these tasks is a foundation of a fresh Cosmos SDK blockchain so that you don't have to write it yourself. To build your blockchain foundation, use the `tinyport scaffold chain` command.
...
```bash
tinyport scaffold chain github.com/cosmonaut/hello
```
...
In the development environment, you don't have to restart the blockchain after you make changes. Because Tinyport server hot reloads, all changes you make in the `hello` directory files are automatically detected.

## Say "Hello, Tinyport"

To get your Cosmos SDK blockchain to say `Hello! Tinyport`, you need to make these changes:
...
```bash
tinyport scaffold query posts --response title,body
```
...
Let's examine some of these changes. For clarity, the following code blocks do not show the placeholder comments that Tinyport uses to scaffold code. Don't delete these placeholders since they are required to continue using Tinyport's scaffolding functionality.
...
func (k Keeper) Posts(c context.Context, req *types.QueryPostsRequest) (*types.QueryPostsResponse, error) {
  //...
  return &types.QueryPostsResponse{Title: "Hello!", Body: "Tinyport"}, nil
}
```
