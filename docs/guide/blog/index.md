...
This series of blog tutorials is based on a specific version of Tinyport, so to install Tinyport v0.19.2 use the following command:
...
```bash
tinyport scaffold chain github.com/cosmonaut/blog
```
...
```bash
tinyport scaffold message createPost title body
```
...
Next, look at the `x/blog/handler.go` file. Tinyport has added a `case` to the `switch` statement inside the `NewHandler` function. This switch statement is responsible for routing messages and calling specific keeper methods based on the type of the message:
...
When you define the `Post` type in a proto file, Tinyport (with the help of `protoc`) takes care of generating the required Go files.
