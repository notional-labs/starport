...
- `tinyport` for the `cosmosclient` blockchain client
...
require (
	...
	github.com/notional-labs/tinyport v0.19.2
)
...
func main() {
	...
	// account `alice` was initialized during `tinyport chain serve`
	accountName := "alice"
...
[cosmosclient](https://pkg.go.dev/github.com/notional-labs/tinyport/tinyport/pkg/cosmosclient) package documentation. Details are provided to learn how to use the `Client` type with `Options` and `KeyringBackend`.

## Running the Blockchain and the Client

Make sure your blog blockchain is still running with `tinyport chain serve`.
