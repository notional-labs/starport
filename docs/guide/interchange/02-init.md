...
```bash
tinyport scaffold chain github.com/cosmonaut/interchange --no-module
```
...
```bash
tinyport scaffold module dex --ibc --ordering unordered --dep bank
```
...
Run the following Tinyport `type` commands to create `sellOrderBook` and `buyOrderBook` types:

```bash
tinyport scaffold map sell-order-book amountDenom priceDenom --no-message --module dex
tinyport scaffold map buy-order-book amountDenom priceDenom --no-message --module dex
```
...
```bash
tinyport scaffold packet create-pair sourceDenom targetDenom --module dex
tinyport scaffold packet sell-order amountDenom amount:int priceDenom price:int --ack remainingAmount:int,gain:int --module dex
tinyport scaffold packet buy-order amountDenom amount:int priceDenom price:int --ack remainingAmount:int,purchase:int --module dex
```
...
```go
tinyport scaffold message cancel-sell-order port channel amountDenom priceDenom orderID:int --desc "Cancel a sell order" --module dex
tinyport scaffold message cancel-buy-order port channel amountDenom priceDenom orderID:int --desc "Cancel a buy order" --module dex
```
...
```go
tinyport scaffold map denom-trace port channel origin --no-message --module dex
```
