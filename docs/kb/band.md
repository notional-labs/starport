...
```bash
tinyport scaffold band [queryName] --module [moduleName]
```
...
The following command scaffolds the IBC-enabled oracle. by default, the tinyport scaffold oracle for [coin rates](https://laozi-testnet4.cosmoscan.io/oracle-script/37#bridge) request and result.

```shell
$ tinyport scaffold chain github.com/cosmonaut/oracle --no-module && cd oracle 
$ tinyport scaffold module consuming --ibc
$ tinyport scaffold band coinRates --module consuming
```
...
In another tab, configure and run the tinyport relayer.
```shell
...
```shell
$ tinyport scaffold band goldPrice --module consuming
```
