---
order: 2
description: Use the Tinyport serve command to start your blockchain.
---
...
Switch to the directory that contains a blockchain that was scaffolded with Tinyport. To start the blockchain node, run the following command:
...
Because the `tinyport chain serve` command is a development tool, do not use it in a production environment. Read on to learn the process of running a blockchain in production.

## The magic of `tinyport chain serve`

The `tinyport chain serve` command starts a fully operational blockchain.

The `tinyport chain serve` command performs the following tasks:
...
Flags for the `tinyport chain serve` command determine how your blockchain starts. All flags are optional.
...
The `tinyport chain serve` and `tinyport chain build` commands compile the source code of the chain in a binary file and install the binary in `~/go/bin`. By default, the binary name is the name of the repository appended with `d`. For example, if you scaffold a chain using `tinyport scaffold chain github.com/alice/chain`, then the binary is named `chaind`.
