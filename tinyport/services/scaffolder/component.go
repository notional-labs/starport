...
import (
	...
	"github.com/notional-labs/tinyport/tinyport/pkg/protoanalysis"
...
func checkForbiddenComponentName(name multiformatname.Name) error {
	...
	case
		...
		"tx",
		datatype.TypeCustom:
		return fmt.Errorf("%s is used by Tinyport scaffolder", name.LowerCamel)
	}
...
// checkComponentCreated checks if the component has been already created with Tinyport in the project
...
func checkForbiddenOracleFieldName(name string) error {
	...
	case
		...
		"PREPAREGAS",
		"EXECUTEGAS":
		return fmt.Errorf("%s is used by Tinyport scaffolder", name)
	}
	return nil
}
