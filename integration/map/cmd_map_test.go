...
func TestCreateMapWithStargate(t *testing.T) {
	...
	env.Must(env.Exec("create a map",
		step.NewSteps(step.New(
			step.Exec("tinyport", "s", "map", "user", "user-id", "email"),
			step.Workdir(path),
		)),
	))
	...
	env.Must(env.Exec("create a map with custom path",
		step.NewSteps(step.New(
			step.Exec("tinyport", "s", "map", "appPath", "email", "--path", filepath.Join(path, "app")),
	...
	env.Must(env.Exec("create a map with no message",
		step.NewSteps(step.New(
			step.Exec("tinyport", "s", "map", "nomessage", "email", "--no-message"),
			step.Workdir(path),
		)),
	))
	...
	env.Must(env.Exec("create a module",
		step.NewSteps(step.New(
			step.Exec("tinyport", "s", "module", "example", "--require-registration"),
			step.Workdir(path),
		)),
	))
	...
	env.Must(env.Exec("should prevent creating a map with a typename that already exist",
		step.NewSteps(step.New(
			step.Exec("tinyport", "s", "map", "user", "email", "--module", "example"),
			step.Workdir(path),
		)),
	...
	env.Must(env.Exec("create a map in a custom module",
		step.NewSteps(step.New(
			step.Exec("tinyport", "s", "map", "mapUser", "email", "--module", "example"),
			step.Workdir(path),
		)),
	))
	...
	env.Must(env.Exec("create a map with a custom field type",
		step.NewSteps(step.New(
			step.Exec("tinyport", "s", "map", "mapDetail", "user:MapUser", "--module", "example"),
			step.Workdir(path),
		)),
	))
	...
	env.Must(env.Exec("create a message and a map with no-message flag to check conflicts",
		step.NewSteps(step.New(
			step.Exec("tinyport", "s", "message", "create-scavenge", "description"),
			step.Exec("tinyport", "s", "map", "scavenge", "description", "--no-message"),
			step.Workdir(path),
		)),
	))
	...
	env.Must(env.Exec("should prevent creating a map with duplicated indexes",
		step.NewSteps(step.New(
			step.Exec("tinyport", "s", "map", "map_with_duplicated_index", "email", "--index", "foo,foo"),
			step.Workdir(path),
		)),
		envtest.ExecShouldError(),
	))
	...
	env.Must(env.Exec("should prevent creating a map with an index present in fields",
		step.NewSteps(step.New(
			step.Exec("tinyport", "s", "map", "map_with_invalid_index", "email", "--index", "email"),
			step.Workdir(path),
		)),
		envtest.ExecShouldError(),
	))
